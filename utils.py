...
def linesearch(x_old: np.ndarray, f_old:float , grad: np.ndarray, search_direction: np.ndarray, x: np.ndarray, f:float, step_max:float, check: bool, func: Callable):
    ...
    """
    ALF = 1e-4
    TOLX = np.finfo(float).eps  # ALF ensures sufficient decrease in function value, TOLX is the convergence criterion on \nabla x
    ...
    if sum > step_max:
        search_direction = search_direction * step_max/sum # rescale if attempted step is too big
    
    slope = np.dot(grad, search_direction) # slope = dot product of gradient and search direction 
    ...
    while True:
        ...
        if a_lambda < a_lambda_min:
            ...
            if a_lambda == 1.0:
                ...
                a = (rhs1 / (a_lambda ** 2) - rhs2 / (a_lambda_2**2)) / (a_lambda - a_lambda_2)
                b = (-a_lambda_2 * rhs1 / (a_lambda **2) + a_lambda * rhs2 / (a_lambda_2 **2)) / (a_lambda - a_lambda_2)
                if a == 0.0:
        ...
        a_lambda_2 = a_lambda
        f2 = f
        a_lambda = np.maximum(temp_lambda, 0.1 * a_lambda) # lambda is at least 10% of the old lambda
...
class Funcd(Generic[T]):
    ...
    def df(self, x: np.ndarray):
        ...
        for ix in range(0,n):
            temp = x[ix]
            h = self.EPS * math.fabs(temp) # temp is a float, no need to introduce numpy
            if h == 0.0:
                h = self.EPS
            xh[ix] = temp + h
            h = xh[ix] - temp
